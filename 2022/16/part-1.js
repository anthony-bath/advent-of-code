...
const valves = readOld(YEAR, DAY, PART).map((line) => {
...
function dfs(state) {
  if (state.minutes <= 0 || state.openedValves.length === valvesWithFlow.length) {
    return state.flow;
  }
  ...
  const possibleValves = valvesWithFlow
    .filter(
      ({ name }) =>
        !state.openedValves.includes(name) && state.minutes > distance[state.valve.name][name] + 1
    )
...
write(YEAR, DAY, PART, dfs(inititalState));
