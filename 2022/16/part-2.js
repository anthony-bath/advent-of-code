export function part2({ lines }) {
  ...
  for (const [ourOpenValves, elephantOpenValves] of openValvePairs) {
    ...
    const us = dfs(inititalState, ourOpenValves);

    const possibleBest =
      us +
      elephantOpenValves.reduce(
        (acc, valve) => acc + (26 - distance['AA'][valve] - 1) * valvesByName.get(valve).flowRate,
        0
      );
