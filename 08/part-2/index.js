...
inputs.forEach((input, index) => {
  ...
  const middleAndTopLeftSegments = codeByDigit
    .get(4)
    .replace(oneExp, "")
    .split("");

  const fourSevenExp = new RegExp(
    `[${codeByDigit.get(4)}${codeByDigit.get(7)}]`,
    "g"
  );

  const bottomAndBottomLeftSegments = codeByDigit
    .get(8)
    .replace(fourSevenExp, "")
    .split("");
  ...
  _069.forEach((digit) => {
    ...
    const missing = codeByDigit
      .get(8)
      .replace(new RegExp(`[${digit}]`, "g"), "");
...
fs.writeFileSync(
  "./08/part-2/output.txt",
  displayed.reduce((sum, current) => sum + current, 0).toString()
);
