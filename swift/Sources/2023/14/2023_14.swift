...
extension Year2023 {
  struct Day14: AdventDay {
    ...
    func loadData() -> Year2023.Day14Data {
      var yMax = 0
      var xMax = 0
    ...
    func tiltNorth(_ allStones: [Year2023.Stone], _ xMax: Int, _ blockersByColumn: [Int: [Int]]) {
      for x in 0 ..< xMax {
    ...
    func tiltWest(_ allStones: [Year2023.Stone], _ yMax: Int, _ blockersByRow: [Int: [Int]]) {
      for y in 0 ..< yMax {
    ...
    func tiltSouth(
      _ allStones: [Year2023.Stone],
      _ xMax: Int,
      _ yMax: Int,
      _ blockersByColumn: [Int: [Int]]
    ) {
      for x in 0 ..< xMax {
        let stones = allStones.filter { $0.x == x }.sorted { $0.y > $1.y }
        guard !stones.isEmpty else { continue }
    ...
    func calcLoad(allStones: [Year2023.Stone], yMax: Int) -> Int {
      allStones.reduce(0) { $0 + (yMax - $1.y) }
    }

    func getKey(allStones: [Year2023.Stone], load: Int) -> String {
