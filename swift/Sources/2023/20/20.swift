...
extension Year2023 {
  struct Day20: AdventDay {
    ...
    func part1() -> Any {
      let network = Year2023.Network(lines: lines)
      var (low, high) = (1000, 0)

      for _ in 1 ... 1000 {
        var queue = [(Year2023.Pulse.LOW, "broadcaster", "button")]
        ...
        while queue.count > 0 {
          ...
          if pulses.first!.0 == Year2023.Pulse.LOW {
            low += pulses.count
          } else {
            high += pulses.count
          }
      ...
      while rxInputInputs.count > 0 {
        ...
        while queue.count > 0 {
          ...
          for pulse in pulses {
            ...
            if rxInputInputs.contains(target) && type == Year2023.Pulse.LOW {
