...
for (const [dir, point] of [folds[0]]) {
  ...
  switch (dir) {
    case "y":
      ...
      paperAfterFold = [...Array(yMaxNew + 1)].map((_) =>
        Array(xMax + 1).fill(".")
      );
    ...
    case "x":
      ...
      paperAfterFold = [...Array(yMax + 1)].map((_) =>
        Array(xMaxNew + 1).fill(".")
      );
      ...
      for (let y = 0; y < yMax + 1; y++) {
        for (let i = 1; i < xMax - point + 1; i++) {
          ...
          paperAfterFold[y][newX] =
            paper[y][newX] === "#" || paper[y][oldX] === "#" ? "#" : ".";
        }
      }
...
fs.writeFileSync(
  "./13/part-1/output.txt",
  paper
    .reduce((count, row) => row.filter((c) => c === "#").length + count, 0)
    .toString()
);
