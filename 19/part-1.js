...
const blueprints = read(19).map((line) => {
  const {
    id,
    oreRobotOreCost,
    clayRobotOreCost,
...
write(
  ...
  1,
  `${allGeodes.reduce(
    (totalQualityLevel, geodes, index) =>
      totalQualityLevel + geodes * (index + 1),
    0
  )}`
);
...
function dfs(state, blueprint, cache) {
  ...
  if (
    state.resources[TYPE.GEODE] +
      state.minutes * (state.robots[TYPE.GEODE] + state.minutes) <
    maxSoFar
  ) {
    return 0;
  }
  ...
  if (robots[TYPE.ORE] < maxRobots[TYPE.ORE]) {
    const waitTime = Math.max(
      0,
      Math.ceil(
        (blueprint.oreRobotOreCost - resources[TYPE.ORE]) / robots[TYPE.ORE]
      )
    );
  ...
  if (robots[TYPE.CLAY] < maxRobots[TYPE.CLAY]) {
    const waitTime = Math.max(
      0,
      Math.ceil(
        (blueprint.clayRobotOreCost - resources[TYPE.ORE]) / robots[TYPE.ORE]
      )
    );
  ...
  if (
    robots[TYPE.OBSIDIAN] < maxRobots[TYPE.OBSIDIAN] &&
    robots[TYPE.CLAY] > 0
  ) {
    const oreRequiredTime = Math.max(
      0,
      Math.ceil(
        (blueprint.obsidianRobotOreCost - resources[TYPE.ORE]) /
          robots[TYPE.ORE]
      )
    );
    ...
    const clayRequiredTime = Math.max(
      0,
      Math.ceil(
        (blueprint.obsidianRobotClayCost - resources[TYPE.CLAY]) /
          robots[TYPE.CLAY]
      )
    );
  ...
  if (robots[TYPE.OBSIDIAN] > 0) {
    const oreRequiredTime = Math.max(
      0,
      Math.ceil(
        (blueprint.geodeRobotOreCost - resources[TYPE.ORE]) / robots[TYPE.ORE]
      )
    );
    ...
    const obsidianRequiredTime = Math.max(
      0,
      Math.ceil(
        (blueprint.geodeRobotObsidianCost - resources[TYPE.OBSIDIAN]) /
          robots[TYPE.OBSIDIAN]
      )
    );
  ...
  options.forEach(([waitTime, botType, cost]) => {
    const nextState = {
      ...
      resources: [
        resources[TYPE.ORE] +
          (waitTime + 1) * robots[TYPE.ORE] -
          cost[TYPE.ORE],
        resources[TYPE.CLAY] +
          (waitTime + 1) * robots[TYPE.CLAY] -
          cost[TYPE.CLAY],
        resources[TYPE.OBSIDIAN] +
          (waitTime + 1) * robots[TYPE.OBSIDIAN] -
          cost[TYPE.OBSIDIAN],
