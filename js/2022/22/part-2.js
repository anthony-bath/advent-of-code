import { getNextDirection } from './common.js';

export function part2({ lines }) {
  const SIZE = 50;
  const WIDTH = Math.max(...lines.filter((line) => !/\d/.test(line)).map((line) => line.length));
  ...
  return 1000 * (row + 1) + 4 * (col + 1) + DIRECTION_SCORE[direction];
}
